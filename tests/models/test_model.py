...
def test_model_dim():
    ...
    batch = tokenizer(test_texts, padding='max_length', truncation=True, max_length=input_dim,add_special_tokens=False,return_tensors="pt")
  
    
    assert model_output(tokenizer.vocab_size,input_dim,batch).shape == (len(test_texts),1)
